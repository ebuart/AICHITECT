...
EXPERIENCE_OVERHAUL (DEC-0017, control/10, 2026-07-05 — CURRENT PRIMARY WORK): user raised the bar to industry-standard ("no AI-feel, real doing, visual"). control/10 = binding VX voice rules + IX doctrine (feel-before-quiz, one-concept-many-uses anatomy, screenshot loop mandatory) + per-arc treatment map. SHIPPED: voice ratchet test, RequestFlowExplorer flagship (`explorer` block kind), NODE-01-03 re-anatomied as pilot, ARC-00/01 + Home re-authored, EN pilot matched. NEXT per rollout order: ARC-02 live context window → ARC-05 retrieval bench → ARC-08 attack replay → ARC-07 eval dashboard → ARC-03/04 → ARC-09 repo explorer. Every arc = voice re-author + native flagship + screenshot iteration. Tests 238/238.
