...
RESUME_AUDIT (2026-07-04/05): publication layer SHIPPED + polished (DEC-0015/0016, FL-0067..0072) — public repo github.com/ebuart/AICHITECT, LIVE at aichitect-theta.vercel.app (auto-deploy from main), English README with screenshots, MIT, ESLint 0 errors (react-hooks compiler lints at ERROR incl. buildgame after FL-0071), CI green, DE/EN chrome + ARC-00 EN content pilot. OQ-0014 fully resolved except (i) save-wipe doc note. OQ-0015 (a)-(c) content fixes DONE (FL-0069) + lesson-level quality gates; (d) sources-surface + (e) translation sequencing remain. NEW: OQ-0016 = external validity audit (vs 2026 hiring data, OWASP LLM/agentic Top 10, TryHackMe model) — coverage validated strong; gaps = RAG-poisoning/embedding-security/cost-DoS nodes, the "real reps" ceiling (one REAL directed build as capstone bridge), ops thinness, optional shareable mastery export. Tests 229/229.

EXPERIENCE_OVERHAUL (DEC-0017, control/10, 2026-07-05 — CURRENT PRIMARY WORK): user raised the bar to industry-standard ("no AI-feel, real doing, visual"). control/10 = binding VX voice rules (incl. VX-B1 impersonal register, VX-T10 no Netflix narration) + IX doctrine + THE PROTOCOL GRAMMAR IX-10..18 (required-not-optional · dossier-first force-read · raw artifacts, guard-tested · reading free/answering explicit · reveal-on-solve · one AUFGABE panel · watch-primers that never name the target · findings board as artifact · mechanics hints once). Earned through 4 user-test rounds on the NODE-01-03 pilot (FL-0074..0080) — READ control/10 BEFORE building any interactive; these mistakes are documented so they are not repeated. SHIPPED: voice ratchet (em-dash/genau/maxim/du ceilings), RequestFlowExplorer protocol, dossier block (force-read Akte), NODE-01-03 pilot complete, ARC-00/01 + Home re-authored, EN pilot matched. ARC-14 SYSTEM PHYSICS opened (OQ-0017, the senior-gap answer): NODE-14-01 Under Load SHIPPED (LoadSim: queue road + live Little's Law; laws pinned in tests/loadSim.test.ts); next physics node = latency budgets (OQ-0017c). Rollout continues: ARC-02 live context window → ARC-05 retrieval bench → ARC-08 attack replay → physics nodes interleaved. VIS rules (Skilljar bar) in control/10 Part 2b. Tests 254/254.
