...
const waitText = (snippet) => page.waitForFunction((s) => document.body.innerText.includes(s), { timeout: 30000 }, snippet)
...
await wait(300)
// Run 1
await click('Lauf starten'); await wait(6000); await shot('load-run1-mid')
await waitText('dieselbe Zahl wie „Neu/s"'); await solve('Fertig/s'); await shot('load-run1-solved')
await click('Weiter zu Lauf 2'); await wait(250)
// Run 2
await click('Lauf starten'); await wait(4800); await shot('load-run2-queue')
await waitText('ohne je zu sinken'); await shot('load-run2-diagnose'); await solve('Warteschlange')
await click('Weiter zu Lauf 3'); await wait(250)
// Run 3
await click('Lauf starten'); await waitText('zusätzliche Arbeit selbst erzeugt'); await shot('load-run3-retry'); await solve('Zugänge/s')
...
// Run 5
await click('Lauf starten'); await waitText('warum es trotzdem immer wieder Staus gibt'); await solve('Neu/s'); await shot('load-final')
await click('Protokoll abschließen'); await wait(500); await shot('load-free')
await browser.close()
console.log('done')
